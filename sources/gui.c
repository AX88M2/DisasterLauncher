...
void gui_draw_bg(void)
{
	...
	SDL_FRect src = { ((int)bgFrame % 16) * 120, 0, 120, 120 };
	SDL_FRect dst = { 0, 0, 120, 120 };
	for (int i = -1; i < 8; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			...
			SDL_RenderTexture(gRenderer, texture, &src, &dst);
		}
	}
...
boolean gui_init(void)
{
	...
	gWindow = SDL_CreateWindow("Disaster Launcher (build " __DATE__ " " __TIME__ ")", 960, 540, 0);
	...
	if (gRenderer == NULL) {
		ILOG("Failed to created renderer");
		return FALSE;
	}
	...
	euSock = sock_open(STATUS_SERVER, 12084);
	if (!euSock)
		return FALSE;
