...
static void image_paint(image_t* image)
{
	SDL_FRect dst = {
		image->x,
		image->y,
		image->src.w * image->scale,
		image->src.h * image->scale
	};
...
image_t* image_create_ext(const string filename, char* tag)
{
	...
	SDL_GetTextureSize(image->texture, &image->w, &image->h);

	image->src = (SDL_FRect){ 0, 0, image->w, image->h };
	image->scale = 1;
	
	if(tag) {
		strcpy_s(image->tag, 32, tag);
	}
